...
输出: outputs/{领域词}_{时间戳}/
      ├── {领域词}_{时间戳}_数据源清单.csv   ← 唯一交付物（UTF-8 BOM，Excel 直接打开）
      ├── {领域词}_{时间戳}_stats.csv        ← 效果数据（多次运行横向对比）
      ├── {领域词}_{时间戳}_搜索日志.csv      ← 搜索复盘（每次搜索的查询词/提取数）
      ├── {领域词}_{时间戳}_raw.json         ← 中间产物归档（过滤前全量，调试/复盘）
      └── {领域词}_{时间戳}_证据留痕.jsonl    ← 会话级搜索证据留痕（调试/复盘）
```
...
搜索层  .claude/skills/autosource-search/SKILL.md  ← 验证搜索 + 增量发现，返回候选
...
```bash
python -m pytest tests/ -q   # 77 个测试，预期全过
```
