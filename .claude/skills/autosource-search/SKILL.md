...
- 每条候选声明 `granularity`：**合集级**（默认）/ **站点级** / **单篇级**——仅作信息标注（后续站点/子站合并功能会用到），脚本不据此拒绝任何条目
- 判定话术（提取时自行把握）：
  1. "这个 URL 背后是持续更新/完整的知识体系，还是一次性内容？"
  2. "这个来源在领域内有没有合集入口？有 → 优先收合集入口。"
- 单篇级在 `reason` 中写明依据（如"团体标准文件，无合集可替代"）
