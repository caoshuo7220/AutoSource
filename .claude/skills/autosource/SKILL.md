...
**纪律**：证据核对不手工做——禁止用 grep/shell 循环复合命令核对 URL 是否在留痕中（这是脚本的职责，且复合命令会触发权限弹窗）；直接运行本命令，脚本拒绝后按恢复路径处理（自动保留 raw.json 与留痕 → 修正 → 重跑本命令）。调查/排查用 Read 工具，不用 shell 循环。
...
- **有被拒条目（证据校验或粒度矛盾）时不生成输出目录**——不写任何 CSV、不归档；stdout 打印被拒明细，`raw.json` 与证据留痕自动保留，修正后重跑本命令即可补入。**全部通过才生成运行目录**（一次调用至多一个目录，且必然是完整版）
...
- 全部通过时归档中间产物：`raw.json` 与证据留痕拷贝进运行目录（`{领域词}_{时间戳}_raw.json`、`{领域词}_{时间戳}_证据留痕.jsonl`，调试/复盘用；证据留痕为会话级文件，含本会话全部搜索记录）——随后删除 `outputs/raw.json` 与留痕临时文件
...
清单核对: 验证通过 X/Y 项
未验证清单（人工交接单）: {列表或"无"}
输出: {脚本 stdout 中的输出目录，若为"未生成"则本轮无产出，按被拒明细修正后重跑}/...
```
