...
- **常态预期（自查标尺）**：合格查询返回的 10 条结果里通常只有 1-2 条需要拒收（无关噪声 / 通用平台首页 / 已收重复）——其余应收：合集入口收合集级，具体文档、报告、文章收单篇级；整批结果确为无关内容（书商、元器件站、SEO 聚合页）时如实提取 0，不为凑数收录垃圾
...
- 知网、专利库、百科、标准平台等跨领域通用平台的**首页/根域名**不提取为独立条目——无主题边界，URL 背后不是该领域的成体系内容
...
`source_type` 是内容形态标签：如实描述该来源是什么形态（如"厂商文档""市场研究报告""开源项目"），不填机构名、领域名或网址。
...
- **首轮优先官方入口**：验证搜索的目标是官方入口 URL——首轮查询词优先用"来源识别 + 官网 / 官方文档"（如 `东土科技 官网`、`Aruba techdocs`、`Nokia 官方文档`）；暂不使用"来源识别 + 具体体裁词"（如 `新华三 配置手册`）——这类词易命中垃圾页，首轮落空由扩量轮换角度补救。官网式首轮也可能落空——落空即标记未验证，留扩量轮换角度，不重复相同查询词重试。
...
- **扩充（第 1 批必跑，至多 2 批，每节点上限 24 次）**：基底 16 次完成后，**必须先执行第 1 批扩充**（4 次，角度池或实体选题均可）；本批完成后按批级判定是否续第 2 批——**续批条件**：本批提取中通过平台准入判据且非重复的**有效新增 ≥ 4 条** → 执行第 2 批；有效新增 0-3 条 → 扩充结束，进入阶段 4。第 2 批完成后扩充结束，不再续批。**与阶段 4 扩量轮区分**：扩充是基底后的高产延续（每节点必经），扩量轮是阶段 4 对薄弱节点的定向补搜——两者不是一回事，扩充不得并入扩量轮执行。
